...
from open_in_cloud_workflow.replace_links_in_markdown import (
    __main__ as replace_links_in_markdown_main,
...
def test_replace_links_in_markdown_via_markdown_tag(
    ...
    """Test link replacement for a markdown tag in markdown-only notebook."""
    ...
    assert (
        ...
        == "[Link to the main notebook](Link to main_notebook.ipynb)"
    )
...
def test_replace_links_in_markdown_via_html_tag_single_quotes(
    ...
    """Test link replacement for HTML links with single quotes."""
    ...
    assert (
        updated_cells[0].source
        == "<a href='Link to main_notebook.ipynb'>Link to the main notebook</a>"
    )
...
def test_replace_links_in_markdown_via_html_tag_double_quotes(
    ...
    """Test link replacement for HTML links with double quotes."""
...
def test_replace_links_in_markdown_with_a_code_cell(
    ...
    """Test link replacement in notebook containing markdown and code."""
...
def test_replace_links_in_markdown_main(
    ...
    data_subdirectory = os.path.join(
        "tests", "data", "replace_links_in_markdown"
    )
    pattern = os.path.join(data_subdirectory, "*.ipynb")
    test_notebooks: dict[str, dict[type[PublishOnBaseClass], str]] = {
        "markdown_link": {
            PublishOnArtifact: (
            ...
            PublishOnGitHub: (
                ...
                + "open-in-colab/tests/data/replace_links_in_markdown/"
                + "main_notebook.ipynb)"
            ),
        },
        "html_link_single_quotes": {
            PublishOnArtifact: (
                "<a href='main_notebook.ipynb'>"
    ...
    with tempfile.TemporaryDirectory(dir=root_directory) as tmp_root_directory:
        ...
        replace_links_in_markdown_main(
            tmp_root_directory, pattern, "colab", publisher
        )
