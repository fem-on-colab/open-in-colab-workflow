...
def test_get_colab_drive_url_existing(root_directory: str) -> None:
    """Test Google Colab URL for a file already uploaded on Google Drive."""
    data_directory = os.path.join(root_directory, "tests", "data")
    absolute_path = os.path.join(
        data_directory, "upload_files_to_google_drive", "existing_file.txt"
    )
...
def test_get_drive_url_new(root_directory: str) -> None:
    """Test Google Colab URL for a file never uploaded on Google Drive."""
    data_directory = os.path.join(root_directory, "tests", "data")
    data_subdirectory = os.path.join(
        data_directory, "upload_files_to_google_drive"
    )
    with tempfile.NamedTemporaryFile(dir=data_subdirectory) as tmp:
        relative_path = os.path.relpath(tmp.name, root_directory)
        url = get_colab_drive_url(
            relative_path, "GitHub/open_in_colab_workflow"
        )
        assert url is None
