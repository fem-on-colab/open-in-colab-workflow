...
from open_in_cloud_workflow.replace_images_in_markdown import (
    __main__ as replace_images_in_markdown_main,
...
def test_replace_images_in_markdown_via_markdown_tag(
    ...
    """Test image replacement for markdown tag in markdown-only notebook."""
...
def test_replace_images_in_markdown_via_html_tag(
    ...
    """Test image replacement for HTML tag in markdown-only notebook."""
...
def test_replace_images_in_markdown_via_html_and_markdown_tags(
    ...
    """Test image replacement with mixed HTML and markdown tags."""
...
def test_replace_images_in_markdown_with_a_code_cell(
    ...
    """Test image replacement in a notebook with markdown and code."""
...
def test_replace_images_in_markdown_main(
    ...
    data_subdirectory = os.path.join(
        "tests", "data", "replace_images_in_markdown"
    )
...
![Red](data:image/png;base64""",
    ...
    with tempfile.TemporaryDirectory(dir=root_directory) as tmp_root_directory:
        os.makedirs(
            os.path.join(tmp_root_directory, data_subdirectory, "images")
        )
        # Copy test notebooks and images
        for nb_name in test_notebooks.keys():
            shutil.copyfile(
                os.path.join(
                    root_directory, data_subdirectory, f"{nb_name}.ipynb"
                ),
                os.path.join(
                    tmp_root_directory, data_subdirectory, f"{nb_name}.ipynb"
                ),
            )
        for img_file in os.listdir(
            os.path.join(root_directory, data_subdirectory, "images")
        ):
            shutil.copyfile(
                os.path.join(
                    root_directory, data_subdirectory, "images", img_file
                ),
                os.path.join(
                    tmp_root_directory, data_subdirectory, "images", img_file
                ),
            )
        ...
        for nb_name, expected in test_notebooks.items():
            original_nb = open_notebook(
                "replace_images_in_markdown",
                nb_name,
                os.path.join(root_directory, "tests", "data"),
            )
            updated_nb = open_notebook(
                "replace_images_in_markdown",
                nb_name,
                os.path.join(tmp_root_directory, "tests", "data"),
            )
            ...
            for c, expected_c in enumerate(expected):
                if expected_c is None:  # code cell
                    ...
                    assert (
                        updated_nb.cells[c].source
                        == original_nb.cells[c].source
                    )
                else:
