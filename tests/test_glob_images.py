...
def test_glob_images(root_directory: str) -> None:
    """Test that images are transformed to base64 correctly."""
    nb_directory = os.path.join(
        root_directory, "tests", "data", "replace_images_in_markdown"
    )
    ...
    red_base64_trail = [
        "BJQwAAAABJRU5ErkJggg==",
        "tgSUMAAAAASUVORK5CYII=",
        "PodQAAAABJRU5ErkJggg==",
    ]
    ...
    assert any(
        [
            red_base64_trail_ in images_as_base64[red]
            for red_base64_trail_ in red_base64_trail
        ]
    )
    assert blue in images_as_base64
    assert any(
        [
            blue_base64_trail_ in images_as_base64[blue]
            for blue_base64_trail_ in blue_base64_trail
        ]
    )

    # Expected *_base64_trail values can be regenerated by uncommenting
    # the following lines.
    """
    print()
    ...
    # Run test on a dirty repository. Red and blue are found twice:
    # once with their original extension and once as PNG from conversion.
    images_as_base64 = glob_images(nb_directory)
    ...
    assert any(
        [
            red_base64_trail_ in images_as_base64[red_converted]
            for red_base64_trail_ in red_base64_trail
        ]
    )
    assert blue in images_as_base64
    ...
    assert any(
        [
            blue_base64_trail_ in images_as_base64[blue_converted]
            for blue_base64_trail_ in blue_base64_trail
        ]
    )
