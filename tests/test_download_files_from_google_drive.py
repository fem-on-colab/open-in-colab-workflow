...
"""Tests for the open_in_cloud_workflow.upload_files_from_google_drive package."""
...
from open_in_cloud_workflow.download_files_from_google_drive import download_files_from_google_drive
...
def test_download_files_from_google_drive(root_directory: str) -> None:
    ...
    download_pattern = os.path.join("tests", "data", "download_files_from_google_drive", "*.txt")
    check_pattern = os.path.join("tests", "data", "download_files_from_google_drive", "existing_file.txt")
    with tempfile.TemporaryDirectory(dir=root_directory) as tmp_root_directory:
        os.makedirs(os.path.dirname(os.path.join(tmp_root_directory, download_pattern)))
        download_files_from_google_drive(tmp_root_directory, download_pattern, "GitHub/open_in_colab_workflow")
        url = get_drive_url(check_pattern, "GitHub/open_in_colab_workflow")
        assert url == "https://drive.google.com/open?id=1cvMOE2HbDxzQSK6fH4frs0ifZflDsaJY"
