...
def test_upload_files_to_google_drive_existing(root_directory: str) -> None:
    ...
    pattern = os.path.join(
        "tests", "data", "upload_files_to_google_drive", "existing_file.txt"
    )
    with tempfile.TemporaryDirectory(dir=root_directory) as tmp_root_directory:
        ...
        shutil.copyfile(
            os.path.join(root_directory, pattern),
            os.path.join(tmp_root_directory, pattern),
        )
        with open(os.path.join(tmp_root_directory, pattern), "a") as f:
            f.write(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        upload_files_to_google_drive(
            tmp_root_directory, pattern, "GitHub/open_in_colab_workflow"
        )
        url = get_drive_url(pattern, "GitHub/open_in_colab_workflow")
        assert (
            url
            == "https://drive.google.com/open?id=1MUq5LVW4ScYDE1f1sHRi3XDupYe5jOra"
        )
...
def test_upload_files_to_google_drive_new_single_upload(
    root_directory: str,
) -> None:
    """Test uploading a new file on Google Drive."""
    original_pattern = os.path.join(
        "tests", "data", "upload_files_to_google_drive", "new_file.txt"
    )
    with tempfile.NamedTemporaryFile(
        dir=os.path.join(root_directory, os.path.dirname(original_pattern)),
        suffix=".txt",
    ) as tmp:
        shutil.copyfile(
            os.path.join(root_directory, original_pattern), tmp.name
        )
        pattern = os.path.relpath(tmp.name, root_directory)
        upload_files_to_google_drive(
            root_directory, pattern, "GitHub/open_in_colab_workflow"
        )
        url = get_drive_url(pattern, "GitHub/open_in_colab_workflow")
        ...
        delete_command = (
            "rclone -q deletefile "
            f"drive:{os.path.join('GitHub/open_in_colab_workflow', pattern)}"
        )
...
def test_upload_files_to_google_drive_new_double_upload(
    root_directory: str,
) -> None:
    """Test uploading a new file on Google Drive, then replacing it."""
    copy_pattern = os.path.join(
        "tests", "data", "upload_files_to_google_drive", "new_file.txt"
    )
    upload_pattern = os.path.join(os.path.dirname(copy_pattern), "*.txt")
    # Do the first upload
    with tempfile.NamedTemporaryFile(
        dir=os.path.join(root_directory, os.path.dirname(upload_pattern)),
        suffix=".txt",
    ) as tmp1:
        shutil.copyfile(os.path.join(root_directory, copy_pattern), tmp1.name)
        pattern1 = os.path.relpath(tmp1.name, root_directory)
        upload_files_to_google_drive(
            root_directory, upload_pattern, "GitHub/open_in_colab_workflow"
        )
    ...
    with tempfile.NamedTemporaryFile(
        ...
        delete_command = (
            "rclone -q deletefile "
            f"drive:{os.path.join('GitHub/open_in_colab_workflow', pattern2)}"
        )
        subprocess.check_call(
            delete_command.split(" "),
            stdout=subprocess.DEVNULL,
