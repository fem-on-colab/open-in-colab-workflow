...
def add_installation_cells(
    ...
    cloud_provider: str,
    fem_on_cloud_packages_str: str,
    pip_packages_str: str,
) -> tuple[list[nbformat.NotebookNode], list[int]]:
    """Add installation cells on top of the notebook.

    Returns updated notebook content and list of insertions.
    """
    (
        fem_on_cloud_packages_name,
        fem_on_cloud_packages_version,
        fem_on_cloud_packages_url,
        fem_on_cloud_packages_import,
        fem_on_cloud_packages_dependent_imports,
    ...
    (
        ...
        pip_packages_url,
        pip_packages_import,
        pip_packages_dependent_imports,
    ...
    packages_dependent_imports = (
        fem_on_cloud_packages_dependent_imports + pip_packages_dependent_imports
    )

    need_installation_cell = {
        package_import: False for package_import in packages_import
    }
    for package_import, package_dependent_imports in zip(
        packages_import, packages_dependent_imports
    ):
        for cell in nb_cells:
            if cell.cell_type == "code":
                if _package_is_imported(package_import, cell):
                    ...
                    break
                elif package_dependent_imports != "":
                    dependent_imports = package_dependent_imports.split(" ")
                    if any(
                        _package_is_imported(package_dependent_import, cell)
                        for package_dependent_import in dependent_imports
                    ):
                        need_installation_cell[package_import] = True
                        break
    ...
    for package_name, package_install_code, package_import in zip(
        ...
        if need_installation_cell[package_import]:
            package_install_cell = nbformat.v4.new_code_cell(
                package_install_code
            )
            package_install_cell.id = (
                package_name.replace(" ", "_") + "_install"
            )
            updated_nb_cells.insert(
                first_code_cell_position, package_install_cell
            )
...
def _package_is_imported(
    package_import: str, cell: nbformat.NotebookNode
) -> bool:
    """Auxiliary function to determine if the cell contains an import."""
    return (
        f"import {package_import}" in cell.source
        or f"from {package_import}" in cell.source
    )
...
def __main__(  # noqa: N807
    ...
    nb_pattern: str,
    cloud_provider: str,
    fem_on_cloud_packages: str,
    pip_packages: str,
) -> None:
    """Add installation cells on top of every notebook in the work directory."""
    for nb_filename in glob_files(work_dir, nb_pattern):
        with open(nb_filename) as f:
            nb = nbformat.read(f, as_version=4)
        nb.cells, _ = add_installation_cells(
            nb.cells, cloud_provider, fem_on_cloud_packages, pip_packages
        )
