...
def get_fem_on_cloud_installation_line(
    ...
    if package_url == "":
        package_url_prefix = (
            f"https://fem-on-{cloud_provider}.github.io/releases"
        )
    else:
        assert "https" not in package_url, (
            "Please provide the commit SHA instead of the full URL"
        )
        if package_url == "current":
            package_url = get_git_head_hash(
                f"https://github.com/fem-on-{cloud_provider}/fem-on-{cloud_provider}.github.io.git",
                "gh-pages",
            )
        package_url_prefix = (
            f"https://github.com/fem-on-{cloud_provider}/"
            f"fem-on-{cloud_provider}.github.io/raw/{package_url}/releases"
        )
    package_url_suffix = (
        ".sh" if package_version == "" else f"-{package_version}.sh"
    )
    package_url = (
        f"{package_url_prefix}/{package_name}-install{package_url_suffix}"
    )
    ...
    return (
        f'wget "{package_url}" -O "/tmp/{package_install}" '
        f'&& bash "/tmp/{package_install}"'
    )
