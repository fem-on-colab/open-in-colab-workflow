...
def get_pip_installation_line(
    ...
    if any(operator in package_version for operator in extras_operators):
        assert all(
            [operator not in package_version for operator in versions_operators]
        )
        install_arg = ""
    elif any(operator in package_version for operator in versions_operators):
        assert all(
            [operator not in package_version for operator in extras_operators]
        )
        install_arg = "--upgrade "
    else:
    ...
    if package_url == "":
        ...
        if package_version != "":
            assert all(
                [
                    operator not in package_version
                    for operator in versions_operators
                ]
            )
            assert any(
                operator in package_version for operator in extras_operators
            )
        if package_url.endswith("@current"):
            package_url_without_commit = package_url.replace("@current", "")
            package_url = (
                package_url_without_commit
                + "@"
                + get_git_head_hash(package_url_without_commit, "HEAD")
            )
        package_url = (
            f'{install_arg}"{package_name}{package_version}@git+{package_url}"'
        )
    pip_installation_line = f"python3 -m pip install {package_url}"
    if package_extra_commands_before_install != "":
        ...
        pip_installation_line = (
            f"{package_extra_commands_before_install} "
            f"&& {pip_installation_line}"
        )
